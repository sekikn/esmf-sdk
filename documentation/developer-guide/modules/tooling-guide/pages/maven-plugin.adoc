...
| `channelAddress` | Sets the channel address (i.e., for MQTT, the topic's name). Each of the
  `tenant-id`, `namespace`, `version` and `aspect-name` placeholders present in the address is
  rendered as a channel parameter in the generated specification; if a placeholder is missing from
  the address, its parameter is omitted. | `String` | `/\{tenant-id}/{namespace}/{version}/{aspect-name}` | {nok}
| `useSemanticApiVersion` | Use the complete semantic version of the Aspect Model as the version of the Aspect API. | `Boolean` | `false` | {nok}
