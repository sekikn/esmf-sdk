...
This includes the generated success response `200` and the generated error responses `400`, `401`, `403` and `404`.

- If `responses` is specified, the template defines the non-success responses.
Generated error responses are not added automatically.

- The generated `200` success response is still added automatically when the template does not define `responses."200"`.
...
- If the template defines `responses."200"`, the template response replaces the generated `200`.

- The operation-level vendor extension `x-esmf-generate-success-response: false` disables adding the generated `200`.
It only affects the generated success response.
Responses explicitly defined in the template, including `200`, `201`, `204` or any other response code, are kept.
...
To provide reusable common error responses for generated GET operations that use the operation template,
specify the error responses in the template.
The generated `200` response is preserved and reusable common error responses can be provided by reference:
...
To replace the generated `200` success response, define `responses."200"` explicitly:
...
To use another success status code without adding the generated `200`, disable the generated success response:
