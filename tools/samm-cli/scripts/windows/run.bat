...
if /i "%HostProcessName%"=="explorer.exe" (
  cd /d "%ExeWorkingDir%"
  for %%i in (%~dp0samm-cli-*.jar) do %~dp0runtime\jre\bin\java --enable-native-access=ALL-UNNAMED --sun-misc-unsafe-memory-access=allow -Dpolyglotimpl.DisableMultiReleaseCheck=true -jar "%%i"
  cmd /k
) else (
  for %%i in (%~dp0samm-cli-*.jar) do %~dp0runtime\jre\bin\java --enable-native-access=ALL-UNNAMED --sun-misc-unsafe-memory-access=allow -Dpolyglotimpl.DisableMultiReleaseCheck=true -jar "%%i" %*
)
